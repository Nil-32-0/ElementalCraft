package metafact.elementalcraft.block.shrine.ore;
...
import metafact.elementalcraft.loot.LootHelper;
...
public class OreShrineBlockEntity extends AbstractShrineBlockEntity {
    ...
    private static int getFortuneLevel(AbstractShrineBlockEntity shrine) {
        ...
        if (direction != null) {
            ...
            if (shrine.getLevel() == null) return fortune;
            var runeHandler = shrine.getLevel().getCapability(ElementalCraftCapabilities.RuneHandler.BLOCK, direction.getOpposite());
//            var runeHandler = shrine.getCapability(ElementalCraftCapabilities.RUNE_HANDLE);


            if (!runeHandler.isPresent()) return fortune;

            var runeHandlerExposed = runeHandler.orElseThrow(() -> new RuntimeException("How did you reach this?"));

            if (runeHandlerExposed.getRuneCount(Runes.TZEENTCH) > 0) {
                fortune++;
            }
        }
        return fortune;
