...
public class ECCreativeModeTabs {
    ...
    public static final RegistryObject<CreativeModeTab> ELEMENTAL_CRAFT_CREATIVE_TAB = DEFERRED_REGISTER.register("elemental_craft", () -> CreativeModeTab.builder()
            ...
            .displayItems((p, o) -> {
                ...
                ECBlocks.RESERVOIRS.values().forEach(reservoir -> generateElementContainer(o, reservoir));
                ...
                ECBlocks.SOURCE_DISPLACEMENT_PLATES.values().forEach(plate -> o.accept(plate.get()));
                o.accept(ECBlocks.SOURCE_BREEDER.get());
                o.accept(ECBlocks.SOURCE_BREEDER_PEDESTAL.get());
                ECItems.ARTIFICIAL_SOURCE_SEEDS.values().forEach(seed -> o.accept(seed.get()));
                ECItems.NATURAL_SOURCE_SEEDS.values().forEach(seed -> o.accept(seed.get()));
                ...
                ECBlocks.CRYSTAL_BLOCKS.values().forEach(block -> o.accept(block.get()));
                ECItems.CRYSTALS.values().forEach(crystal -> o.accept(crystal.get()));
                o.accept(ECItems.PURE_CRYSTAL.get());
                ECItems.SHARDS.values().forEach(shard -> o.accept(shard.get()));
                ECItems.POWERFUL_SHARDS.values().forEach(shard -> o.accept(shard.get()));
                ...
                ECItems.LENSES.values().forEach(lens -> o.accept(lens.get()));
