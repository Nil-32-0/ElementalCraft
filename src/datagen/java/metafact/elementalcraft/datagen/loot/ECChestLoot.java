package metafact.elementalcraft.datagen.loot;
...
import metafact.elementalcraft.loot.function.RandomSpell;
...
public class ECChestLoot implements LootTableSubProvider {
	...
	private static LootPool.Builder createAdvancedPool(ElementType type) {
		return LootPool.lootPool().name("advanced").setRolls(UniformGenerator.between(1, 3))
				...
				.add(rune(getLargeRune(type)).setWeight(5))
	...
	private static ResourceKey<Rune> getLargeRune(ElementType type) {
		return switch (type) {
			case AIR -> Runes.ZOD;
