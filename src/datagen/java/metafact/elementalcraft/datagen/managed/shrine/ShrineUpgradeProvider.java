package metafact.elementalcraft.datagen.managed.shrine;
...
import metafact.elementalcraft.data.predicate.block.shrine.HasShrineUpgradePredicate;
...
public class ShrineUpgradeProvider extends AbstractManagedDataBuilderProvider<ShrineUpgrade, ShrineUpgrade.Builder> {
    ...
    private static final List<ResourceKey<ShrineUpgrade>> ADVANCED_UPGRADES = List.of(
            ShrineUpgrades.OVERCLOCKED_ACCELERATION, ShrineUpgrades.TRANSLOCATION, ShrineUpgrades.GREATER_FORTUNE,
            ShrineUpgrades.OVERWHELMING_STRENGTH
    );
	...
	protected void collectBuilders(HolderLookup.Provider registries) {
        ...
        advancedShrineUpgrade(ShrineUpgrades.OVERCLOCKED_ACCELERATION).predicate(ACCELERATION_PREDICATE)
                ...
                .addBonus(BonusType.ELEMENT_CONSUMPTION, 1.5F);
        advancedShrineUpgrade(ShrineUpgrades.GREATER_FORTUNE).predicate(miningPredicate(ECTags.Blocks.SHRINES_UPGRADABLES_FORTUNE))
    ...
    protected ShrineUpgrade.Builder advancedShrineUpgrade(ResourceKey<ShrineUpgrade> key) {
        return shrineUpgrade(key).max(1).incompatibleWith(ADVANCED_UPGRADES);
    }
